...
class BaseLoader(Dataset):
    ...
    def chunk(self, frames, bvps, spo2, chunk_length, video_type):
        ...
        return np.array(frames_clips), np.array(bvps_clips) if bvps_clips is not None else None, np.array(spo2_clips) if spo2_clips is not None else None
    ...
    def load_preprocessed_data(self):
        ...
        labels_bvp = [input_file.replace("face_input", "face_hr") for input_file in inputs_only_face]
        labels_spo2 = [input_file.replace("face_input", "face_spo2") for input_file in inputs_only_face]
